...
```json
{
  ...
  "tasks": [
    { "id": "uuid", "type": "daily", "title": "倒垃圾", "note": "可回收與廚餘分開", "tags": ["家務"],
      "modules": [ { "type": "step", "id": "uuid", "title": "分類", "done": true },
                   { "type": "progress", "current": 3, "target": 10, "step": 1 } ],
...
- 新增／編輯任務的 sheet 右下角有一顆同款 ＋（`#sheet-fab`）：**短按或長按都開輪盤**，選項「新增進度條」
  （日常限定）與「新增步驟」。**每選一次就在「預定日程」下方的模塊列表最後接一個模塊，先加的在上**；
  步驟可加多個，進度條最多一個。每個模塊右上角 − 移除（進度條與有名稱的步驟會 `confirm`）。
- **進度條**：目前進度、目標進度、每次增加（預設 1）。**步驟**（＝子任務）：左側圓圈點一下切換完成／未完成，
  右側輸入名稱。步驟**只在點進任務的 sheet 裡看得到**，卡片上刻意不顯示；卡片只顯示進度「進度 3 / 10」。
- **模塊在 sheet 裡就必須填完整**才能建立／儲存（`A.checkProgressInput`、步驟名稱不得空白）：任一欄空白會提示
  「補填或按 − 移除」並把焦點移到第一個有問題的欄位。
- 資料：`task.modules` 有序陣列，元素是 `{ type:'progress', current, target, step }` 或
  `{ type:'step', id, title, done }`；順序就是顯示順序。`normModules` 把壞的個別丟掉、一般任務的進度條丟掉，
  **v7 的 `task.progress` 讀入時自動轉成進度條模塊**（放最前面），`schema_version` 不變。
  daily-tick 不認得 `modules`，由那邊寫回會被丟掉（同 `tags`）。
- sheet 開著時 DOM 是模塊的唯一真相（`main.js` 的 `buildModule` / `collectModules`），儲存時才收回資料，
  所以打字、勾選都不會重繪。
...
- **進度隨完成連動**（`A.bumpProgress`，在 `A.toggle` 裡）：完成任務 → `current + step`（不超過 `target`）、
  取消完成 → `current − step`（不低於 0）。日常任務每期完成算一次。邊界會夾值，所以在邊界來回切換
  不保證完全對稱（9→10→8）——刻意取捨，不為對稱多存一個未夾值欄位。
- 還沒定的：步驟全完成跟任務完成的關係、進度到目標是否自動完成。
...
node ui.test.mjs                 # 瀏覽器行為：手勢、編輯模式、週期、軟刪除、鍵盤、離線、版面、卡片分割與主題、篩選、排序、FAB 長按面板、輪盤手勢與搜尋、模塊、sheet 捲動、Apple 式打磨、小面板與 token、鍵盤露出欄位（422 項）
